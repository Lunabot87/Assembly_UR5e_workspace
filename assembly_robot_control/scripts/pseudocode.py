####################################################
# 하나의 step이 시작할때 callback function 호출
####################################################
const double kHoleCheckOffset = 0.28 # 애들이 정함
...
def assembly_cb(msg):
	for i in range(len(msg.asms)):
		switch (msg.asms[i].type){
			case 'insert_pin':
				insert_pin(msg.asms[i])
			case 'insert_part':
				insert_part(msg.asms[i])
			case 'screw':
				screw(msg.asms[i])
			case 'flip':
				flip(msg.asms[i])
...
##############################################
# asm.msg + grasp_config.yaml -> grasp.msg
# asm.msg -> insert.msg (?)
class Lv3(Lv2):
	def __init__(self):
