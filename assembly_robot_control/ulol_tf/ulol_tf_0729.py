...
class TF_Node(object):
	...
	def add_pin(self, pin_type):
		...
		if pin_has_hole[pin_type]:
			...
			pin_hole_pose = self.set_hole_pose(pin_origin_pose,pin_type,0,pin_hole_offset)
	...
	def set_hole_pose(self,origin_pose,part_num,hole_num,hole_offset):
		...
		return hole_pose
	def set_grasping_pose(self,part_num,point_num):
	...
	def get_part_pose(self,mesh_name):
		...
		for hole_num in range(num_of_holes):
			...
			hole_pose = self.set_hole_pose(mesh_pose.pose,part_num,hole_num,hole_data)
		...
		for gp in range(num_of_gp):
			grasping_pose = self.set_grasping_pose(part_num,gp)
			self.GP_List[part_num]['pose'].append(copy.deepcopy(grasping_pose))

	def get_pin_pose(self,pin_type,pin_tag): #Pin_name = pin_name[pin_number]+"-"+str(pin_exist+1)

		target_pin_name = pin_name[pin_type]+"-"+str(pin_tag+1)
		target_pin_pose = geometry_msgs.msg.PoseStamped()
		target_pin_pose.header = self.scene.get_objects([target_pin_name])[target_pin_name].header
		target_pin_pose.pose = self.scene.get_objects([target_pin_name])[target_pin_name].mesh_poses[0]

		if pin_has_hole[pin_type]:
			pin_origin_pose = copy.deepcopy(target_pin_pose.pose)
			...
			pin_pose = copy.deepcopy(target_pin_pose.pose)
		self.Pin_List[pin_type]['pose'].append(pin_pose)
	...
	def update_tf(self):
		...
		for pin_type in range(4):
			for pin_tag in range(how_many_pins[pin_type]):
				self.get_pin_pose(pin_type,pin_tag)
